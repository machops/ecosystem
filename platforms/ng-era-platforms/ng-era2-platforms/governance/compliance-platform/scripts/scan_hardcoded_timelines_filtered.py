...
class FilteredTimelineScanner:
    ...
    def should_skip_file(self, file_path):
        ...
        if any(pattern in file_path.name.lower() for pattern in 
               ['backup', 'archive', 'old', 'legacy', 'temp', 'tmp']):
            return True
    ...
    def save_report(self, output_path):
        ...
        # 保存 JSON 報告
        json_path = Path(output_path) / 'timeline-scan-filtered.json'
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(report, f, indent=2, ensure_ascii=False)
        
        # 保存 Markdown 報告
        md_path = Path(output_path) / 'timeline-scan-filtered.md'
    ...
    def _save_markdown_report(self, report, output_path):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write("# 硬編時間線掃描報告（過濾版本）\n\n")
            f.write(f"**掃描時間**: {report['scan_time']}\n")
            f.write("**過濾條件**: 排除備份目錄、歷史檔案、暫時文件\n\n")
...
def main():
    """主函數"""
    print("開始掃描儲存庫中的硬編時間線（過濾版本）...")
    print("=" * 60)
    
    scanner = FilteredTimelineScanner()
    print(f"掃描目錄: {scanner.repo_path}")
    print("過濾條件: 排除備份目錄、歷史檔案、暫時文件")
