...
ROOT_FILES = [
    ...
    ".root.init.d/03-gl-platform.governance-init.sh",
...
def _run_init_scripts(root_path: Path, env: Dict[str, str]) -> List[Dict[str, Any]]:
    ...
    bootstrap_cmd = f"source \"{bootstrap_script}\" >/dev/null 2>&1"
...
def _build_environment(repo_root: Path, root_path: Path, semantic_version: str) -> Dict[str, str]:
    ...
    env.update({
        ...
        "GOVERNANCE_REGISTRY": str(root_path / ".root.jobs/gl-platform.governance-registry.json"),
        ...
        "GOVERNANCE_CONFIG": str(repo_root / "controlplane/config/root.gl-platform.governance.yaml"),
